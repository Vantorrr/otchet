...
async def generate_brief_pptx(settings, period_data, prev_data, period_name, start, end, daily_series):
    """Generate brief-compliant PPTX and return bytes."""
    # Re-run main logic but return bytes instead of saving to file
    from bot.services.yandex_gpt import YandexGPTService
    from bot.services.presentation import PresentationService
    import io
    ...
    totals = prs_service._calculate_totals(period_data)
    prev_totals = prs_service._calculate_totals(prev_data) if prev_data else {}
    avg = prs_service._calculate_average_manager(period_data)
    
    # Build presentation (copy logic from main() above)
    # ... implementation using existing code above ...
    
    # For now, use the old presentation service as fallback
    pptx_bytes = await prs_service.generate_presentation(
        period_data, period_name, start, end, prev_data, 
        prev_data and list(prev_data.values())[0] if prev_data else None,
        prev_data and list(prev_data.values())[0] if prev_data else None
    )
    return pptx_bytes
